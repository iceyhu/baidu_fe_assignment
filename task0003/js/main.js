require(['util'], function(util){
    ...
    var uniqArray = util.uniqArray;
    var trim = util.trim;
    var isValidDate = util.isValidDate;
    var showInfo = util.showInfo;
    var resizeToWindowSize = util.resizeToWindowSize;
    var replaceContentHtml = util.replaceContentHtml;
    var validateInput = util.validateInput;
    var dateDescending = util.dateDescending;
    ...
    */
    $.delegateByClassName('#content', 'save', 'click', function(){
        ...
        switch (contentHtmlUtil.isEditing) {
            // 处于‘编辑’状态
            case true:
                if (validateInput(taskLib, false)) {
                    // 保存要修改的task的引用
